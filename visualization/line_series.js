...
function updateSessionStatus(stat) {
  document.getElementById("SessionStatus").innerHTML = stat;
  if (stat == "Active") {
    document.getElementById("SessionStatus").color = "green";
  }
  else {
    document.getElementById("SessionStatus").color = "red";
  }
}
...
function render() {
  ...
  var log = d3.csv(config.LOGS_SRC + SessionID+'.csv' + '?' + Math.floor(Math.random() * 100000000000), function(d, i) {
    ...
    if (LogEntriesSeen != comm.length) {
      reset();
      for (var i = 0; i < comm.length; ++i) {
        update(comm[i]);
      }
      LogEntriesSeen = comm.length;
    }
    SessionStatus = "Active";
    updateSessionStatus(SessionStatus);
  })
